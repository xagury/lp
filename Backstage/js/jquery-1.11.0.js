...
//handleError 方法在jquery1.4.2之后移除了，此处重写改方法 add by yangying for ajaxfileupload
...
//		console.info("options.error="+options.error);
		
		if( typeof(options.error)=='undefined' && typeof(smscPermitDenyAlert) == 'function'){
			options.error = smscPermitDenyAlert;
		}
...
jQuery.fn.extend({
	...
	smscSerialize: function() {
		return jQuery.param( this.smscSerializeArray() );
	},
	smscSerializeArray: function() {
		return this.map(function() {
			// Can add propHook for "elements" to filter or add form elements
			var elements = jQuery.prop( this, "elements" );
			return elements ? jQuery.makeArray( elements ) : this;
		...
		.map(function( i, elem ) {
			var val = jQuery( this ).val();
			if( val!=null ){
				if( typeof($(elem).attr('datatype')) != 'undefined'){
					if($(elem).attr('datatype')=='Date'){
						//format string to long
						var d = to1970(val);
						//console.info(d.getTime());
						return { name: elem.name, value: d!=null?d:null };
					}
				}
			}
			return val == null ?
